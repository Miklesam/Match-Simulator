...
import com.miklesam.dotamatchsimulator.utils.hideKeyboard
...
class FragmentClient : Fragment(R.layout.fragment_client) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        clientViewModel.getToastMessage().observe(viewLifecycleOwner, Observer {
            Toast.makeText(context, it, Toast.LENGTH_LONG).show()
        })

        clientViewModel.getProgress().observe(viewLifecycleOwner, Observer {
            when (it) {
                0 -> {
